# -*- coding: utf-8 -*-
"""
Created on Wed Dec  7 22:51:34 2022
...
B.append([
          ...
          XL[2][1]-XL[1][1],
          XL[-1][1]-XL[2][1]])
W=[(400-(Ga-Gd-SommeTeta))*10**4,-R0cc*(XA[0]-XL[-1][0]),-R0cc*(XA[1]-XL[-1][1])]
...
sigmaLL= np.linalg.inv(sigmaL)-196*Qv
sigma0es=(V_T*P*V)/4

Vbeta=np.matrix([V[0,0],
              V[1,0],
              V[2,0],
              V[3,0]])*10**-4

VDcc=np.matrix([[V[4,0]],
              [V[5,0]],
              [V[6,0]]]
              )
VDcm=np.matrix([VDcc[i,0]/R0cc*distance[i] for i in range(len(distance))])
LD=np.matrix(distance).transpose()+VDcm.transpose()
Lbeta=np.matrix(angle).transpose()+Vbeta.transpose()

tetaic=[ ]
for i in range(len(angle)):
    tetaic.append((Lbeta[i]-200))
SommeTetac=sum(tetaic)
#gisement en grades
gisementc=[Gd]
for i in range(len(angle)):
    gisementc.append(gisementc[i]+tetai[i])
XLc=[XD]
for i in range(len(angle)-1):
    XLc.append([XLc[i][0]+LD[i,0]*singr(gisementc[i+1]),
                XLc[i][1]+LD[i,0]*cosgr(gisementc[i+1])])