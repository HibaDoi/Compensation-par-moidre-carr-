...
def singr(x):
    return sin(x*pi/200)
#Gisement de départ et d'arrivée 

Gd=round(Gt(XRD[0],XRD[1],XD[0],XD[1]),4)

Ga=round(Gt(XA[0],XA[1],XRA[0],XRA[1]),4)

#gisement approchéeon ajout dans la loste le qisement de départ 
alphaij=[Gd]
for i in range(len(angle)-2):
    alphaij.append(round(rp(alphaij[i]+angle[i]+200),4))

#on ajout les coodonnée de départ du point de départ 
co_APP=[[XD[0],XD[1]]]
...
co_APP.append(XA)
co_APP.append(XRA)
#list des gisements et distances calclculer par coordonnée    
alpha0ij=[round(Gd,4)]
D0=[]
for i in range(len(angle)-1):
    D0.append(round(d(co_APP[i][0],co_APP[i][1],co_APP[i+1][0],co_APP[i+1][1]),4))
